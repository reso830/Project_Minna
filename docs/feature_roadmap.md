...
| `005-state-transitions` | `0.8.0` | Planned | Operator-triggered work item state transitions (Start quick phrase for parked items, status chip hover dropdown, Close reason selection modal replacing Drop) with single work_item.state_changed event persistence in local SQLite and 422 error handling. |
